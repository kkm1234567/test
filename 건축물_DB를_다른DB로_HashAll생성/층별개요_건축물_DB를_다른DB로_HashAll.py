...
def deduplicate_with_hash(dst_conn, dst_table):
    ...
    # Dictionary to store: hash -> (row_data, count)
    hash_dict = {}
    processed = 0
    start = time.time()

    print("Phase 1: Reading destination table data and calculating hashes...")
    while True:
        row = read_cursor.fetchone()
        if row is None:
            break

        # Calculate hash from all columns (excluding BuildingFloorCd, DuplicationRecodeCnt, CreateDateTime, OriginDocumentMonth)
        hash_data = []
        ...
        if hash_value in hash_dict:
            ...
            # Create new row with hash as BuildingFloorCd
            final_row = list(row)
            building_floor_cd_idx = dst_col_names.index('BuildingFloorCd')
            final_row[building_floor_cd_idx] = hash_value
            # Initialize DuplicationRecodeCnt to 1
            dup_count_idx = dst_col_names.index('DuplicationRecodeCnt')
            final_row[dup_count_idx] = 1
        ...
        if processed % 100000 == 0:
            print(f"  → Processed {processed:,}/{total_rows:,} rows, unique hashes: {len(hash_dict):,}")
    ...
    for hash_value, data in hash_dict.items():
        ...
        # Update DuplicationRecodeCnt with actual count
        dup_count_idx = dst_col_names.index('DuplicationRecodeCnt')
        row_list[dup_count_idx] = data['count']
