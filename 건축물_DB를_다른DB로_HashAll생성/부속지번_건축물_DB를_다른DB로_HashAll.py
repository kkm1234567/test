...
# -----------------------------------------------
# Configuration for tCollectorBuildingSubLot
# -----------------------------------------------
DST_TABLE = "tCollectorBuildingSubLot"
PK_COLS = ("BuildingLedgerCd", "BuildingSubLotCd")
...
def deduplicate_with_hash(dst_conn):
    """Hash-based deduplication for tCollectorBuildingSubLot"""
    ...
    print(f"총 {total_rows:,} rows 읽어서 hash 계산 중...")
    ...
    while True:
        ...
        for row in rows:
            ...
            # Build hash string from non-skip columns
            hash_parts = []
            ...
            # Create row with hash as PK2
            final_row = list(row)
            final_row[pk2_idx] = hash_value
            if dup_idx is not None:
                final_row[dup_idx] = 1
            
            # Store in dict: hash -> {row, count}
    ...
    # Phase 2: Truncate and reinsert unique records
