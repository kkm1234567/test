...
TABLE_NAME = "건축물대장_지역지구구역"
OUTPUT_DIR = r"C:\PTR\Prime\Collect\CollectApi\src\storage\hub_go_kr\t_collector_building_region_district_section\202104"
OUTPUT_FILE = "건축물대장_지역지구구역.txt"
BATCH_SIZE = 5000
LOG_EVERY = 100000
...
def export_to_txt():
    ...
    if not columns:
        print(f"ERROR: 테이블 '{TABLE_NAME}'의 컬럼 정보를 찾을 수 없습니다.")
        print(f"테이블이 존재하는지 확인하세요: [{MSSQL_DATABASE}].[dbo].[{TABLE_NAME}]")
        schema_cursor.close()
        conn.close()
        return
